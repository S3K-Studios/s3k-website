...
const OurStorySection: React.FC = React.memo(() => {
  ...
  React.useEffect(() => {
    if (aboutTimeline.length === 0 && !dataLoading[collections.ABOUT_TIMELINE]) {
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-10 relative z-10">
            {aboutTimeline.map((item, index) => {
              const trans = item.translations?.find((tr: any) => tr.language === currentLang) || item.translations?.[0] || {};
              return (
                <div key={item.id || index} className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                    <span className="text-xs font-bold text-primary uppercase tracking-widest">
                      {item.year} - {trans.badge || t(item.badgeKey)}
                    </span>
