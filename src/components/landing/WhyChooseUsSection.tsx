      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {whyChooseUs.map((feature, index) => {
            const trans = feature.translations?.find((tr: any) => tr.language === currentLang) || feature.translations?.[0] || {};
            return (
              <div key={feature.id || index} className="flex flex-col items-center">
                <Icon className="text-5xl text-primary mb-4" icon={feature.icon} />
                <h3 className="text-xl font-semibold mb-2">
                  {trans.title || t(feature.titleKey)}
                </h3>
                <p className="text-foreground-500">{trans.desc || t(feature.descKey)}</p>
              </div>
            );
          })}
        </div>
