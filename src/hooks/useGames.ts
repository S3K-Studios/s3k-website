...
export const useGames = () => {
    ...
    const { games: rawGames, loading: dataLoading, error: dataError, refreshData } = useData();
    ...
    const localizedGames = useMemo(() => {
        return rawGames.map((g: any) => {
            const trans = g.translations?.find((t: any) => t.language === currentLang) ||
                g.translations?.find((t: any) => t.language === 'tr') ||
                g.translations?.[0] || {};
            return { id: g.id, ...trans };
        });
    }, [rawGames, currentLang]);

    return {
        games: localizedGames as (GameTranslation & { id: string })[],
