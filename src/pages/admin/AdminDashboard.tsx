...
import JsonToolsModal from "../../components/admin/JsonToolsModal";
import {
  collections,
  fetchCollectionData,
  saveDocument,
...
import { games as gamesSeedData } from "../../data/games";
import { blogPosts as blogsSeedData } from "../../data/blogs";
...
const seedDataByCollection: Record<string, any[] | undefined> = {
  [collections.GAMES]: gamesSeedData,
  [collections.BLOGS]: blogsSeedData,
};
    ...
    switch (activeTab) {
        ...
        return (
          <SimpleContentForm
            ...
            translatableFieldsConfig={[
              ...
              { name: "badge", label: "Rozet Metni", type: "text" },
            ]}
            onCancel={() => setIsModalOpen(false)}
            onSubmit={handleSave}
          />
      ...
      case collections.ABOUT_VALUES:
        return (
          <SimpleContentForm
            fieldsConfig={[
              { name: "icon", label: "İkon (örn: material-symbols:groups)" },
              { name: "percentage", label: "Yüzde" },
              { name: "colorClass", label: "Renk Class (bg-blue-500)" },
              { name: "iconBgClass", label: "İkon Arka Plan Class" },
            ]}
            initialData={editingItem}
            translatableFieldsConfig={[
              { name: "title", label: "Başlık", type: "text" },
      ...
      case collections.WHY_CHOOSE_US:
        return (
          <SimpleContentForm
            fieldsConfig={[
              { name: "icon", label: "İkon (örn: lucide:wand-2)" },
            ]}
            initialData={editingItem}
            translatableFieldsConfig={[
              { name: "title", label: "Başlık", type: "text" },
    ...
    <div className="flex h-screen bg-content2">
      ...
      <div className="flex-1 p-8 overflow-y-auto w-full">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold uppercase">
            {activeTab.replace("_", " ")} Yönetimi
          </h1>
          {activeTab !== collections.AUDIT_LOGS && (
            <div className="flex gap-2">
              <Button variant="flat" onClick={() => setIsJsonModalOpen(true)}>
                JSON Araçları
              </Button>
              <Button color="primary" onClick={() => openForm(null)}>
                Yeni Ekle
              </Button>
            </div>
          )}
        ...
        {loading ? (
          ...
          <div className="bg-content1 rounded-lg shadow-md overflow-hidden">
                  ...
                  .map((row) => {
                    ...
                    return (
                      <tr key={row.id}>
                        ...
                        <td
                          className="px-6 py-4 whitespace-nowrap text-sm opacity-70"
                          title={row.adminId}
                        >
                          {row.adminId
                            ? `${row.adminId.substring(0, 8)}...`
                            : "System"}
                        </td>
            ...
            {data.length === 0 && (
              <div className="p-8 text-center opacity-50">Log bulunamadı.</div>
            )}
          </div>
        ) : (
          <DataTable
      ...
      {isJsonModalOpen && (
        <JsonToolsModal
          ...
          seedData={seedDataByCollection[activeTab]}
